...
# --------------------------------------------------
$rows = @(
    [PSCustomObject]@{ Label = $Messages.Splash_Label_Version;     Value = "v$($DefaultSettings.version)" },
    [PSCustomObject]@{ Label = $Messages.Splash_Label_Dev;         Value = "Small Software House" },
    [PSCustomObject]@{ Label = $Messages.Splash_Label_Workflow;    Value = $Messages.Splash_Workflow_Value },
    [PSCustomObject]@{ Label = $Messages.Splash_Label_Credentials; Value = "Google Cloud / Azure" },
    [PSCustomObject]@{ Label = "GitHub";                           Value = "SmallSoftwareHouse/DubbingToolkit" }
)
$maxLen = ($rows | ForEach-Object { $_.Label.Length } | Measure-Object -Maximum).Maximum
Write-Host ""
Write-Host ("=" * 80) -ForegroundColor Cyan
Write-Host ("                           DUBBING TOOLKIT") -ForegroundColor Cyan
Write-Host ("-" * 80) -ForegroundColor Cyan
foreach ($row in $rows) {
    $pad = " " * ($maxLen - $row.Label.Length)
    Write-Host "  $($row.Label)$pad : $($row.Value)"
}
Write-Host ("=" * 80) -ForegroundColor Cyan
Write-Host ""

...
The main dependencies are listed in `Config/dependencies.json`.
...
> **Note:** settings are automatically saved between sessions via `Settings/settings_persistent.json`. The interface language selected during installation is applied on first launch without any further action required.
...
If the project was installed via the distribution package, use the Windows **Apps & Features** panel or the uninstaller in the installation folder.

The uninstaller lets you choose what to permanently delete:
- Credentials and API keys
- Billing data (monthly TTS usage)
- Session logs
- Work files (projects, audio, video, transcripts)

Data not selected is preserved. The virtual environment (`venv/`) is always removed automatically.

### Moving

Although not recommended, the project can be moved to a different location, but after every move the virtual environment must be recreated, as the venv contains absolute paths tied to the original location. Procedure:

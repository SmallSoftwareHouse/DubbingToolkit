...
输出文件位于活动项目文件夹中。在 `Workspace/projects/{项目名称}/` 中查找：
- `audio_extraction/current/` — 提取的音频
- `transcripts/current/` — SRT 转录文件
- `translated/current/` — SRT 翻译文件
- `dubbed/current/` — 最终配音音频

项目根目录中的 `project_info.json` 文件包含每次处理的详细信息。

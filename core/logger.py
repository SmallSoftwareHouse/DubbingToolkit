...
def _windows_product_name() -> str:
    """Return the real Windows product name (e.g. 'Windows 11 Pro') via registry.
    Corrects the Microsoft bug where Windows 11 early builds report 'Windows 10'
    in ProductName — detection is done via CurrentBuild (>= 22000 = Windows 11).
    Falls back to platform.system() + platform.release() on any error."""
    try:
        import winreg
        key = winreg.OpenKey(
        ...
        winreg.CloseKey(key)
        name = str(name)
        # Fix: if registry still says Windows 10 but build >= 22000, it's Windows 11
        if int(build) >= 22000 and "Windows 10" in name:

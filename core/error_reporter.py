...
_REPORTS_DIR  = _LOGS_DIR / "reports"
...
def _build_zip(sessions_paths: list[Path], report_text: str) -> Path:
    ...
    zip_path = _REPORTS_DIR / f"DubbingToolkit_error_report_{ts}.zip"
...
def send_error_report(
    ...
    try:
        ...
        def _m(key: str, *args) -> str:
            """Get a localized string from messages, fall back to key name."""
            val = getattr(messages, key, key) if messages else key
            for i, arg in enumerate(args):
                val = val.replace("{" + str(i) + "}", str(arg))
            return val

        print()
        print(C + "=" * 54 + R)
        print(C + f"  {_m('ErrorReport_Title')}" + R)
        print(C + "=" * 54 + R)
        print()
        print(G + f"  ✓ {_m('ErrorReport_ZipReady')}" + R)
        print(f"    {zip_path.name}")
        print(f"    {_m('ErrorReport_ZipLocation')}")
        print()
        print(C + f"  ─── {_m('ErrorReport_WhatToDo')} " + "─" * (42 - len(_m('ErrorReport_WhatToDo'))) + R)
        print()
        print(Y + "  1." + R + f" {_m('ErrorReport_Step1')}")
        print(Y + "  2." + R + f" {_m('ErrorReport_Step2a')}")
        print(f"     {_m('ErrorReport_Step2b')}")
        print()
        print(Y + f"  3. {_m('ErrorReport_Step3a')}" + R)
        print(Y + f"     {_m('ErrorReport_Step3b')}" + R)
        print()
        print(f"  {_m('ErrorReport_Recipient')} {C}{DEVELOPER_EMAIL}{R}")
        print()
        print(C + f"  {_m('ErrorReport_Contains')}" + R)
        print(f"    • {_m('ErrorReport_Contains1')}")
        print(f"    • {_m('ErrorReport_Contains2', MAX_PAST_SESSIONS)}")
        print(f"    • {_m('ErrorReport_Contains3')}")
        print(f"    • {_m('ErrorReport_Contains4')}")
        print()
        print(C + "=" * 54 + R)
        print()
        input(f"  {_m('ErrorReport_PressEnter')}")
